...
function LineRender({ line }: { line: Line }) {
  ...
  const generateLineCommond = (LQLPoints: Point[]) => {
  ...
  const LQLPoints = getAllKeyPoints(line);
  const roundedPoints = getRoundedPoints(LQLPoints);
  const commond = generateLineCommond(roundedPoints);
  // const dumbPoint = new Point(0, 0);
  // const [firstPoint = dumbPoint] = LQLPoints;
  // const MCommand = `M ${firstPoint.x} ${firstPoint.y} `;
  // const LCommand = LQLPoints.map((P) => ` L ${P.x} ${P.y} `).join("");
    ...
    <>
      <div style={{ position: "absolute" }}>
        <svg width="200" height="200" style={{ overflow: "visible" }}>
          ...
          <path fill="transparent" stroke="black" d={commond} />

          {/* <circle cx="10" cy="10" r="2" fill="red" /> */}
        </svg>
      </div>
      {renderPoints(LQLPoints)}
    </>
  );
}
